# -*- coding: utf-8 -*-
"""Resolve merge conflicts per user choices; write UTF-8 Chinese (via unicode escapes)."""
...
# ---------- 1. pom.xml: no AI ----------
pom = ROOT / "zzyl/pom.xml"
pom_text = pom.read_text(encoding="utf-8")
# strip conflict and AI dependency block
import re
pom_text = re.sub(
    r"<<<<<<< HEAD\n\s*<!-- hutool -->\n=======\n.*?>>>>>>> [^\n]+\n",
    "        <!-- hutool -->\n",
...
# also remove AI if still present without markers
pom_text = re.sub(
    r"\s*<!--[^-]*springai[^-]*-->\s*<dependency>\s*<groupId>com\.alibaba\.cloud\.ai</groupId>.*?</dependency>\s*",
    "\n",
    pom_text,
    flags=re.S | re.I,
)
pom_text = pom_text.replace("<<<<<<< HEAD", "").replace("=======", "")
pom_text = re.sub(r">>>>>>> [^\n]+\n?", "", pom_text)
w("zzyl/pom.xml", pom_text)

# ---------- 2. LeaveController ----------
leave = r'''package com.soft.controller;
 ...
 * \u5217\u8868\u4e2d\u300c\u8bf7\u5047\u4e2d/\u8d85\u65f6\u672a\u5f52\u300d\u6309\u9884\u8ba1\u8fd4\u56de\u65f6\u95f4\u4e0e\u5f53\u524d\u65f6\u95f4\u52a8\u6001\u5224\u5b9a\uff1b\u4e0e\u534f\u540c\u6a21\u5757\u6620\u5c04\u89c1 CollaborationController#mapLeaveFlow\u3002
 */
@RestController
...
public class LeaveController {
    ...
    /**
     * \u8bf7\u5047\u5355\u5206\u9875\u3002
     * status=\u5168\u90e8 \u65f6\u4e0d\u8fc7\u6ee4\uff1b\u8bf7\u5047\u4e2d/\u8d85\u65f6\u672a\u5f52\u6309\u9884\u8ba1\u8fd4\u56de\u65f6\u95f4\u8fc7\u6ee4\uff0c\u5e76\u5728\u8fd4\u56de\u524d\u52a8\u6001\u5237\u65b0\u5c55\u793a\u72b6\u6001\u3002
     */
    @PostMapping("/page")
...
'''
# decode unicode escapes in the string for actual Chinese in file
leave_out = leave.encode("utf-8").decode("unicode_escape").encode("latin1").decode("utf-8")
# The above trick fails for mixed ascii - better use codecs
import codecs
leave_out = codecs.decode(leave, "unicode_escape")
# unicode_escape produces latin1-ish for non-ascii; for \uXXXX it works in Python3 differently
# Safer: leave already has \u in source as real escapes in the r''' - wait r''' doesn't interpret \u
# So leave string has backslash-u. Use codecs.decode:
leave_out = leave.encode("ascii", "backslashreplace").decode("unicode_escape") if False else None
# Correct approach for Python3:
leave_out = leave.encode().decode("unicode_escape")
w("zzyl/src/main/java/com/soft/controller/LeaveController.java", leave_out)

print("LeaveController OK", "\u5f85\u5ba1\u6279" in leave_out)
